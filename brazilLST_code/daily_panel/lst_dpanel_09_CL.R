...
setwd(data_path)
print(igsyear)
gs_id <- ransample[, c("x", "y", "u_id")]
gs_id_qc <- gs_id
# create spatial points data frame for LST extraction
ran_points <- gs_id
coordinates(ran_points)=~x+y
...
gs_id <- merge(gs_id, gs_id_qc, by=c("u_id", "yeardoy"))
#colnames(gs_id)
gs_id <- gs_id[, c("u_id","yeardoy","lst","qc","qcbin1", "qcbin2", "qcbin3")]
setwd(out_path)
save.dta13(gs_id, paste("lst_mhv_4_nw_",igsyear,".dta",sep=""))
}
